...
Esta rama parte de la variante *isend* y añade una única optimización sobre la
...
bloques de palabras espaciadas con la misma clave, que la ruta paralela ya
incorporaba desde *metacache*. El esquema de comunicación no cambia respecto a
*isend*.

Su efecto solo es observable con `np = 1`, ya que con más procesos el programa
ejecuta la ruta paralela, que ya incluía ese precálculo. Al acelerar
precisamente la ejecución de referencia, esta variante hace más conservadoras
las cifras de aceleración, al eliminar del numerador del *speedup* una
ineficiencia de la versión de referencia.
...
- **Streaming patrón a patrón**: las palabras espaciadas se calculan y comunican
  ...
  patrones.
- **Comunicación no bloqueante** (`MPI_Isend` + `MPI_Waitall`), igual que en la
  variante *isend*.
- **Precálculo por patrón en `calc_matches`**: las posiciones *don't-care* y los
  bloques de palabras con la misma clave se calculan una sola vez por patrón, en
  lugar de recalcularse en cada comparación. Solo afecta a la ruta secuencial
  (`np = 1`).
